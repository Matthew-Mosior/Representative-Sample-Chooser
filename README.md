...
The order in which the user provides the field names and corresponding field values is **important**, the first field name specified will be the first field used for comparison, and so on.  The order in which these fields occur in the input TSV file makes no difference.<br/><br/>
